...
const LogglyLogger = require('winston-loggly');
...
class GhostLoggerFactory {
  ...
  constructor (options) {
    ...
    return new GhostLogger(winstonLogger, _.omit(options,["console","loggly","logentries","newrelic"]));
  }
...
class Internals {
  ...
  static InitWinstonLogger (options) {
    ...
    if (_.get(options, "enabled", false)) {
  ...
  static InitLogglyTransport (winstonLogger, options) {
    if (_.get(options, "enabled", false)) {
      ...
      winstonLogger.add(LogglyLogger.Loggly, loggerConfig);
    }
  }
  ...
  static InitLogentriesTransport (winstonLogger, options) {
    if (_.get(options, "enabled", false)) {
      ...
      winstonLogger.add(Winston.transports.Logentries, loggerConfig);
    }
  }
